...
echo "5) ros2 launch mur_launch_sim multi_mur_base.launch.py gz_gui:=true launch_rviz:=true use_simple_visuals:=true"
echo "6) ros2 launch mur_launch_sim multi_mur_base.launch.py gz_gui:=true launch_rviz:=true use_simple_visuals:=true use_wheel_visual_mesh:=true use_laser_visual_mesh:=true"
echo "7) ros2 launch mur_launch_sim multi_mur_base.launch.py gz_gui:=true launch_rviz:=true use_simple_visuals:=true use_top_visual_mesh:=true"
echo "8) ros2 launch mur_launch_sim multi_mur_base.launch.py gz_gui:=true launch_rviz:=true use_simple_visuals:=true use_base_visual_mesh:=true"
echo "9) ros2 launch mur_launch_sim multi_mur_base.launch.py gz_gui:=true launch_rviz:=true use_high_quality_visuals:=true"
...
echo "If run 5 is much faster than launch_rviz:=true with full visuals, RViz RobotModel mesh rendering is the bottleneck."
echo "Runs 6-8 re-enable visual mesh groups while keeping the rest primitive, so they isolate which STL family hurts RViz most."
echo "Run 9 switches base/top from simplified meshes back to full-resolution meshes for screenshot-quality comparison."
